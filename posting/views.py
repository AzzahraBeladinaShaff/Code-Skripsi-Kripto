    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                encryption_duration = end_time - start_time  # Durasi enkripsi dalam detik
                logger.info(f"Encryption duration: {encryption_duration} seconds")  # Tambahkan log untuk durasi
                
                # Setelah enkripsi selesai, simpan file hasil enkripsi
                ...
                # Catat waktu mulai generate kunci RSA
                start_time_rsa_keygen = time.perf_counter()
                logger.info(f"RSA key generation start time: {start_time_rsa_keygen}")
                
                # Enkripsi kunci menggunakan RSA
                public_key, private_key = generate_rsa_keys('posting/primes-to-100k.txt')
                
                # Catat waktu selesai generate kunci RSA
                end_time_rsa_keygen = time.perf_counter()
                rsa_keygen_duration = end_time_rsa_keygen - start_time_rsa_keygen
                logger.info(f"RSA key generation duration: {rsa_keygen_duration} seconds")

                # Catat waktu mulai enkripsi kunci AES menggunakan RSA
                start_time_rsa_encryption = time.perf_counter()
                logger.info(f"RSA encryption start time: {start_time_rsa_encryption}")
                ...
                # Catat waktu selesai enkripsi kunci AES menggunakan RSA
                end_time_rsa_encryption = time.perf_counter()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                # Catat waktu mulai dekripsi RSA
                start_time_rsa_decryption = time.perf_counter()
                logger.info(f"RSA decryption start time: {start_time_rsa_decryption}")
                
                aes_key = rsa_decrypt(private_key, encrypted_key)
                
                # Catat waktu selesai dekripsi RSA
                end_time_rsa_decryption = time.perf_counter()
                rsa_decryption_duration = end_time_rsa_decryption - start_time_rsa_decryption
                logger.info(f"RSA decryption duration: {rsa_decryption_duration} seconds")
                ...
                # Hitung total durasi dekripsi
                # Perbaikan: gunakan decryption_duration yang telah didefinisikan
                total_decryption_duration = rsa_decryption_duration + decryption_duration
                logger.info(f"Total decryption duration: {total_decryption_duration} seconds")
...
def split_secret_sharing(request, encrypted_file_id):
    ...
    try:
        ...
        split_duration = end_time - start_time  # Durasi proses split dalam detik
        logger.info(f"Split duration: {split_duration} seconds")  # Tambahkan log untuk durasi
        ...
        context = {
            ...
            'split_duration': split_duration  # Tambahkan durasi ke context
        }
...
def upload_shares_to_cloud(request, encrypted_file_id):
    ...
    try:
        ...
        file_name = f"{int(time.time())}_{os.path.basename(full_shares_file_paths[1])}"
        with open(full_shares_file_paths[1], 'rb') as share_file:
            dbx.files_upload(share_file.read(), '/' + file_name, mute=True)

        # Upload to OneDrive
        for i in range(2, len(full_shares_file_paths)):
           # Gunakan nama file yang unik dengan timestamp atau UUID
            unique_file_name = f"{int(time.time())}_{os.path.basename(full_shares_file_paths[i])}"
            
            one_drive_upload_url = f"https://graph.microsoft.com/v1.0/me/drive/root:/{unique_file_name}:/content"
...
def fetch_from_google_drive_ajax(request):
    ...
    # Tambahkan query untuk mengecualikan file yang ada di Trash
    query = "trashed = false"
    
    # Ambil file dari Google Drive yang tidak ada di dalam Trash
...
def download_from_onedrive(request, file_id):
    ...
    if response.status_code == 200:
        ...
        if file_metadata_response.status_code == 200:
            ...
            return JsonResponse({'error': 'Failed to fetch file metadata'}, status=file_metadata_response.status_code)
    else:
...
def download_view(request):
    return render(request, 'posting/download.html')
